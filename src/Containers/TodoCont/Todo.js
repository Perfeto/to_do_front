...
class Todo extends React.Component {
	...
	pushNewTodos = async () => {
		...
		try {
			return await fetch('http://127.0.0.1:8085/testapp/todotasks/', {
				method: 'POST',
				headers: headers,
	...
	deleteTodos = (id) => {
		...
		try {
			return fetch('http://127.0.0.1:8085/testapp/todotask/' + id + '/', {
				method: 'DELETE',
				headers: headers
			}).then((res) => {
	...
	updateTodo = () => {
		...
		try{
			return fetch(`http://127.0.0.1:8085/testapp/todotask/${this.state.todoId}/`,{
				method:'PATCH',
				headers: headers,
				body: JSON.stringify(body)
			}).then(res => res.json())
